...
面板分三行：`音乐开关 / 播放选择 / 跟随流程 / 音量`、`导入自选 BGM / 用在`、状态文字。临时切换时「播放选择」直接显示当前临时场景；临时切换和手动锁定时都可点「跟随流程」立即恢复自动。**音乐关闭时只留开关**，其余控件与曲目列表收起；脚本版本过期的提醒仍会显示。
...
1. **下拉框选阶段（手动锁定）**——持续播放所选阶段，入口链接和流程变化都不会切曲。点「跟随流程」或下拉选「自动」解除锁定；锁定期间点过的入口不会在解锁后补播。
2. **故事 / 考察入口链接（临时切换）**——自动模式下，点 回忆突破 / 内蕴奥德赛 / 法洛斯之梦 / 主线 / 特殊事件、冒险中枢 / R&R / 战斗模块，临时播放对应场景。勾选或撤销流程步骤、进入下一天、重置今日、切换 Cycle 或存档后恢复跟随流程，即使前后步骤相同也会恢复。普通界面重绘不会打断临时曲；也可以随时点「跟随流程」结束临时切换。这些入口新开标签，音乐继续由主控台播放。
3. **跟随「今日流程」**（默认）——播放**最后一个已勾选步骤的下一步**。提醒列表不参与进度判断；考察、遭遇等条件步骤即使没有提醒，也不会自动跳过。确实不需要执行的步骤，由用户勾选确认后推进。最后一步完成后播放休整曲。
...
例：勾完「探索」→ 考察；勾完「考察」→ 遭遇；勾完「遭遇」→ 发展（打造与训练）；勾完「故事」→ 灾祸；勾完「灾祸」→ 休整。直接勾选后面的步骤表示确认推进到该步骤之后。

同一首音乐在切换模式、重复选择、重绘或修改其它阶段的自选曲时继续播放，不从头重播。快速连续切曲以最后一次选择为准；离开探索时，其氛围层和转场音也一起淡出。
...
```js
ATO_BGM.setFlowStage("encounter", "campaign:c1:day2:progress"); // 步骤 id + 流程上下文；上下文可省略
ATO_BGM.setStage("mnemos");        // 手动锁定（一直保持）
ATO_BGM.setScene("hub");          // 入口临时切换；锁定时忽略，流程一变撤销
ATO_BGM.setAuto();                 // 清除手动锁定和临时场景，直接跟随今日流程
ATO_BGM.setEnabled(true);          // 开 / 关（默认关闭，必须由用户手势开启）
ATO_BGM.setVolume(0.4);            // 0–1
ATO_BGM.duck(true);                // 压低音乐（保留给朗读场景；story 页目前不调用）
ATO_BGM.state();                   // 当前状态，含 mode（auto/scene/manual）、stepId、loading、missing
...
阶段解析优先级：**手动锁定 > 临时场景 > 今日流程 > 默认阶段**。临时场景不写入偏好，刷新后不会恢复。
...
- 自选曲目在主控台管理；故事页不参与 BGM 播放、导入或切换。
