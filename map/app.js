  ...
  cycleIds.forEach((id) => {
    cycles[id] = {
      ...
      explored: isPlainObject(saved.cycles?.[id]?.explored)
        ...
        : {},
      previewRevealed: isPlainObject(saved.cycles?.[id]?.previewRevealed)
        ? saved.cycles[id].previewRevealed
        : {},
...
function tileFaceId(cycleId, tileId, cycleState = activeCycleState()) {
  if (cycleId !== "c5" || !tileId) return "";
  return cycleState.tileVariants?.[tileId] === "alternate" ? "B" : "A";
}

function tileTagEntry(cycleId, tileId, cycleState = activeCycleState()) {
  const baseEntry = mapTagData.tiles?.[`${cycleId}:${tileId}`] || null;
  const face = tileFaceId(cycleId, tileId, cycleState);
  if (!face) return baseEntry;
  const faceEntry = mapTagData.tiles?.[`${cycleId}:${tileId}:${face}`] || null;
  if (!faceEntry) return baseEntry;
  return {
    ...(baseEntry || {}),
    ...faceEntry,
    factions: Array.isArray(faceEntry.factions) ? faceEntry.factions : (baseEntry?.factions || []),
    factionUpdatedAt: faceEntry.factionUpdatedAt || baseEntry?.factionUpdatedAt || "",
  };
}
...
function tilePreviewRevealLabels(tileId, cycleState = activeCycleState()) {
...
function syncDepartedLandmarkMarkers(cycleState, nextTileId) {
  ...
  if (
    state.activeCycleId === "c4"
    && tileHasTag(state.activeCycleId, departedTileId, "oasis")
  ) {
    moveMarkerToTile(cycleState, "last_oasis", departedTileId);
  }
  if (
    state.activeCycleId === "c5"
    && tileHasTag(state.activeCycleId, departedTileId, "silver_ruin")
  ) {
    moveMarkerToTile(cycleState, "last_silver_ruin", departedTileId);
  }
}
...
function tokenAvailableInCycle(token, cycleId = state.activeCycleId) {
  return Boolean(token) && (
    !Array.isArray(token.cycles)
...
function renderTokenPalette() {
  ...
  if (!tokenAvailableInCycle(tokenAssetById[state.selectedToken])) {
    state.selectedToken = "AG";
  }
  ...
  tokenAssets.filter((token) => !token.group && tokenAvailableInCycle(token)).forEach((token) => {
    fragment.appendChild(renderTokenButton(token));
  });

  tokenMenuGroups.forEach((group) => {
    const groupTokens = tokenAssets.filter((token) => token.group === group.id && tokenAvailableInCycle(token));
...
function renderTiles() {
  ...
  tiles.forEach((tile) => {
    ...
    article.title = tileTooltip(tile, current, explored, hasNote, previewRevealLabels);

    const assets = tileAssets(tile, cycleState);
    const c5Covered = cycle.id === "c5"
      && (state.showBack || (!explored && !previewRevealed));
    const imageSrc = (!state.showBack && (explored || previewRevealed)) || !assets.back ? assets.front : assets.back;
    const imageMarkup = c5Covered
      ? `<span class="tile-blue-cover" role="img" aria-label="未揭示地图 ${escapeHtml(tile.label)}">${escapeHtml(tile.label)}</span>`
...
function tileAssets(tile, cycleState = activeCycleState()) {
  if (state.activeCycleId === "c5" && cycleState.tileVariants?.[tile.id] === "alternate" && tile.back) {
    return {
      ...tile,
      front: tile.back,
      back: tile.front,
...
function renderVariantToggle(tile, cycleState = activeCycleState(), faceVisible = true) {
  const isC1Variant = state.activeCycleId === "c1" && Boolean(c1AlternateTileAssets[tile.id]);
  const isC5DoubleSided = state.activeCycleId === "c5" && Boolean(tile.back);
  if ((!isC1Variant && !isC5DoubleSided) || (isC5DoubleSided && !faceVisible)) return "";
  const alternate = cycleState.tileVariants?.[tile.id] === "alternate";
  if (isC5DoubleSided) {
    const targetSide = alternate ? "A" : "B";
    const currentSide = alternate ? "B" : "A";
    const currentIcon = alternate ? "./images/c5-face-a.png" : "./images/c5-face-b.png";
    return `<button class="tile-variant-toggle c5-face-toggle${alternate ? " active" : ""}" type="button" data-variant-tile="${escapeHtml(tile.id)}" title="切换到 ${targetSide} 面" aria-label="当前 ${currentSide} 面，切换到 ${targetSide} 面"><img src="${currentIcon}" alt="" aria-hidden="true"></button>`;
  }
...
function toggleTileVariant(tileId) {
  const tile = activeCycle().tiles.find((item) => item.id === tileId);
  const canToggleC1 = state.activeCycleId === "c1" && Boolean(c1AlternateTileAssets[tileId]);
  const canToggleC5 = state.activeCycleId === "c5" && Boolean(tile?.back);
  if (!canToggleC1 && !canToggleC5) return;
  const cycleState = activeCycleState();
  pushUndo();
...
function tileTooltip(tile, current, explored, hasNote = false, previewRevealLabels = []) {
  ...
  return [
    ...
    current ? "当前位置" : "",
    explored ? "已探索" : "",
    !explored ? previewRevealLabels.join(" / ") : "",
    hasNote ? "有地图笔记" : "",
    `上 ${neighbors.up || "-"}`,
    `下 ${neighbors.down || "-"}`,
    `左 ${neighbors.left || "-"}`,
...
function placeToken(tileId) {
  clearPendingAdversarySpawn();
  const tokenId = state.selectedToken || "AG";
...
function revealTilePreviewById() {
  ...
  const tile = activeCycle().tiles.find((item) => item.id.toUpperCase() === tileId);
  if (!tile) {
    window.alert(`当前 Cycle 没有板块 ${tileId}。`);
    return;
  }
  const cycleState = activeCycleState();
  if (cycleState.explored[tile.id]) {
    window.alert(`${tile.label} 已探索。`);
    return;
  }
  pushUndo();
...
function currentMapSnapshot() {
  ...
  const lastOasisTileId = Object.keys(cycleState.tokens.markers || {}).find((tileId) => {
    return Boolean(cycleState.tokens.markers?.[tileId]?.last_oasis);
  }) || "";
  const lastOasis = cycle.tiles.find((tile) => tile.id === lastOasisTileId);
  const lastSilverRuinTileId = Object.keys(cycleState.tokens.markers || {}).find((tileId) => {
    return Boolean(cycleState.tokens.markers?.[tileId]?.last_silver_ruin);
  }) || "";
  const lastSilverRuin = cycle.tiles.find((tile) => tile.id === lastSilverRuinTileId);
  const scoutTileId = scoutPlacedTileId(cycleState);
  ...
  const currentTileFace = tileFaceId(cycle.id, cycleState.currentTile, cycleState);
  const latestRevealedTileFace = tileFaceId(cycle.id, cycleState.latestRevealedTile, cycleState);
  ...
  const currentTileHasLastOasisMarker = Boolean(
    cycleState.currentTile && cycleState.tokens.markers?.[cycleState.currentTile]?.last_oasis
  );
  const currentTileHasLastSilverRuinMarker = Boolean(
    cycleState.currentTile && cycleState.tokens.markers?.[cycleState.currentTile]?.last_silver_ruin
  );
  ...
  return {
    ...
    lastOasisTileId,
    lastOasisTileLabel: lastOasis?.label || lastOasisTileId,
    lastSilverRuinTileId,
    lastSilverRuinTileLabel: lastSilverRuin?.label || lastSilverRuinTileId,
    ...
    currentTileHasLastOasisMarker,
    currentTileHasLastSilverRuinMarker,
...
function mapSnapshotNote(snapshot) {
  ...
  return [
    ...
    snapshot.lastOasisTileLabel ? `最后到访的绿洲：${snapshot.lastOasisTileLabel}` : "",
    snapshot.lastSilverRuinTileLabel ? `最后到访的白银遗迹：${snapshot.lastSilverRuinTileLabel}` : "",
...
function visibleTileIdsForUnknownFilter(cycle, cycleState) {
  ...
  (alwaysVisibleStartTileIds[cycle.id] || []).forEach((tileId) => visibleIds.add(tileId));
